 ...
 var shop={
		check:function(a){
			 ...
			 getAttr:function(obj,i,a,l,num,ids){
			  ...
			  if (i==a){
               $("#attrid").val(ids);
			   $("#attrid_b").val(ids); //捆绑套餐
			   try{
			   $('.showDynamicPrice').html(priceunitsymbol +(parseFloat(itemattr[ids][3])*dis).toFixed(2)+ priceunit);
			   $.getScript(dir+'shop/GetStock.aspx?attrid='+ids,function(data){
					   var nowamount=0;
						if (data.indexOf('|')!=-1){  //商品规格按用户组设置价格
								nowamount=parseInt(data.split('|')[0]);
								$('.showDynamicPrice').html(priceunitsymbol +(parseFloat(data.split('|')[1]).toFixed(2))+ priceunit);
						}else{
							   nowamount=parseInt(data);
						}
																					   
					$(".stock").html(nowamount);
				...
				for(var k=0;k<=vlan;k++){
					...
					if (num==2){
						if (itemids.indexOf(",")!=-1){
							 haspro=true;
							 if (bimg!=''){ //图片
							  strs+='<span id="att'+num+k+'" class="txt cloud-zoom-gallery" href="'+bimg+'" rel="useZoom: \'zoom1\', smallImage: \''+bimg+'\' " onclick="shop.getAttr(this,2,'+a+','+vlan+',3,\''+itemids+'\')"><img src="'+simg+'" width=\'25\' height=\'25\' title=\''+iiarr[0]+'\'/><i><img src="'+dir+'sysimg/default/item_sel.gif"/></i></span> ';
							}else{  //文字
						...
						$.ajax({
						   ...
						   success: function (data) {
								...
								if (nowamount>0){
									...
									if (bimg!=''){ //图片
								    	strs+='<span id="att'+num+k+'" class="txt cloud-zoom-gallery" href="'+bimg+'" rel="useZoom: \'zoom1\', smallImage: \''+bimg+'\' " onclick="shop.getAttr(this,2,'+a+','+vlan+',3,\''+itemids+'\')"><img src="'+simg+'" width=\'25\' height=\'25\' title="缺货"/><i><img src="'+dir+'sysimg/default/item_sel.gif"/></i></span> ';
									}else{  //文字
						...
						$.ajax({
						   ...
						   success: function (data) {
								...
								if (nowamount>0){
									haspro=true;
									if (bimg!=''){ //图片
								    	strs+='<span id="att'+num+k+'" class="txt cloud-zoom-gallery" href="'+bimg+'" rel="useZoom: \'zoom1\', smallImage: \''+bimg+'\' " onclick="shop.getAttr(this,3,'+a+','+vlan+',0,\''+itemids+'\')"><img src="'+simg+'" width=\'25\' height=\'25\' title=\''+iiarr[0]+'\'/><i><img src="'+dir+'sysimg/default/item_sel.gif"/></i></span> ';
									}else{  //文字
			  ...
			  if (shop.check(a)){
				...
				jQuery.getScript(dir+'plus/Ajaxs.ashx?BundleIDs='+kbid+'&id='+id+'&attrid='+$("#attrid").val()+'&m='+m+'&a=addCart&num='+$("#num").val(),function(){
				  if (data.flag=='error'){
						    ...
						    alert('对不起，您没有登录!');
							//showuserlogin();
                            window.location.href='/3G/user/login.aspx';
					   }else{
					    //$.dialog({id:'mycart',title:'购物车内商品',max:false,min:false,content:str,left: left});
 ...
 function showBuyInfo(id){
	 jQuery.ajax({
		 type: "POST",
		 url: "/plus/ajaxs.ashx",
		 data: "a=getcartinfo&ismobile=1&proid="+id,
		 success: function(data){
