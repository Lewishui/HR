...
<div class="user_main_r">
          ...
          <div class="right_01 tabbox01" <%if (Utils.GetSysInfo("//sysinfo/model/mall").ToString().ToLower()=="true"){ Response.Write(" style='display:none'");} %>>
             <div class="tit clearfix"><span>我的文章</span>
             <ul class="tabs">
                 <li><a href="#">已审核</a></li>
                 <li><a href="#">待审核</a></li>
                 <li><a href="#">草稿</a></li>
             ...
             <div class="con right_box r_padbox">
          ...
          <div class="r_tybox nomar">
             <div class="tit clearfix" style="border:none; background:#fff">

             <span>我买到的宝贝</span>
                 
                 <span class="small"> 
                     <a href="shop/myorder.aspx?show=-2">待付款(<font><%=DataFactory.GetOnlyValue("select count(1) From KS_ProOrder Where ordertype!=2 and UserName='"+new TemporaryVar().UserName+"' and MoneyReceipt<=0") %></font>)</a>
                     <a href="shop/myorder.aspx?show=-3">待收货(<font><%=DataFactory.GetOnlyValue("select count(1) From KS_ProOrder Where ordertype!=2 and UserName='"+new TemporaryVar().UserName+"' and DeliverStatus=1") %></font>)</a>
                     <a href="shop/mycomment.aspx">待评价(<font><%=DataFactory.GetOnlyValue("select count(1) From KS_ProOrder a inner Join KS_ProOrderItem b on a.OrderID=b.OrderID Where (a.DeliverStatus=2 or a.status=2) and b.IsCmt=0 and a.UserName='" + (new TemporaryVar()).UserName + "'")  %></font>)</a>
                 </span>
                 <span class="small" style="float:right;padding-right:20px;"><a href="Shop/myorder.aspx">所有订单</a></span>

             
             </div>
             <div class="user_main_r_box r_padbox">
       ...
       <asp:Repeater ID="RepeaterOrder" runat="server"  onitemdatabound="RepeaterOrder_ItemDataBound" >
         ...
         <ItemTemplate>
			...
			<tr class="tdbg">
             ...
             <%# Kesion.APPCode.OrderProcess.GetOrderStatus(true,Utils.StrToInt(Eval("orderType")),Convert.ToString(Eval("OrderID")),Utils.StrToInt(Eval("ConsumptionMode")),Utils.StrToInt(Eval("IsBusinessConfirm")),Convert.ToString(Eval("alipaytradestatus")), Convert.ToInt16(Eval("status")), Convert.ToInt16(Eval("DeliverStatus")), Utils.StrToDecimal(Convert.ToString(Eval("MoneyReceipt"))), Utils.StrToDecimal(Convert.ToString(Eval("OrderTotalPrice"))))%>
				...
				<%# ((((float)Convert.ToDouble(Eval("MoneyReceipt")) > 0 &&Convert.ToString(Eval("DeliverStatus")) == "0") || Convert.ToString(Eval("DeliverStatus")) == "2") && Utils.StrToInt(Eval("status"))<=1 && Utils.StrToInt(Eval("IsBusinessConfirm"))==0) ? "<br/><a href='Shop/myorder.aspx?action=returns&id=" + Eval("id") + "'>退货退款</a>" : string.Empty%>
                ...
                <%# !string.IsNullOrEmpty(MyCache.GetShopCommonConfig(42)) && Convert.ToString(Eval("ConsumptionMode")) == "1" && Convert.ToString(Eval("DeliverStatus")) == "1" ? "<br/><a href='Shop/ExpressLog.aspx?OrderID="+Eval("OrderID")+"'>查看物流</a>" : string.Empty %>
                  <br />
                 <%# Kesion.NET.WebSite.User_Order.GetCommentTips(Convert.ToInt16(Eval("ordertype")),Convert.ToInt16(Eval("status")),Convert.ToInt16(Eval("deliverStatus")),Convert.ToString(Eval("orderId"))) %>
                 <%#(Utils.StrToInt(Eval("ordertype"))<=1 && (Utils.StrToInt(Eval("DeliverStatus"))==1||Utils.StrToInt(Eval("DeliverStatus"))==2))?@"<br/><input type=""button"" value=""再次购买"" class=""button"" onclick=""window.open('shop/MyOrder.aspx?action=rebuy&orderid="+ Eval("orderID")+@"')"" />":string.Empty %> 
         ...
         <%} %> 
          ...
          <div class="right_02 tabbox03">
             <div class="tit clearfix"> <span>我的问答</span>
    ...
    <ItemTemplate>
     ...
     <FooterTemplate>
        ...
        <script>
            $(".tabbox01").FengTab({ titCell: ".tabs", mainCell: ".con", defaultIndex: 0, trigger: "mouseover", titOnClassName: "puton", showtime: 0 });
            $(".tabbox03").FengTab({ titCell: ".tabs", mainCell: ".con", defaultIndex: 0, trigger: "mouseover", titOnClassName: "puton", showtime: 0 });
</script>    
...
<%
	  ...
	  string weixinenable = MyCache.GetAccountAPIConfig("weixin", "enable");
      if (qqenable == "true" || sinaenable == "true" || alipayenable == "true" || weixinenable == "true")
      {     
  %>
   
   
   <div class="r_tybox">
	 <div class="r_title"><h4>账号通</h4><a href="Basic/ApiBind.aspx" style="color:#666;font-size:12px">(绑定管理)</a></div>
     	<div class="r_padbox" style="padding-left:0px">
      ...
      if (alipayenable == "true")
      {
	   ...
	   if (weixinenable == "true")
      {
          if (!string.IsNullOrEmpty(__info.WeiXinOpenID))
          {
              sb.AppendFormat(@" <img src=""{{$GetSiteUrl}}sysimg/default/icon_weixin.png"" align=""absmiddle"" title=""{0}""/>{0}", MyCache.GetAccountAPIConfig("weixin", "name"));
          }
          else
          {
              sb.AppendFormat(@" <img src=""{{$GetSiteUrl}}sysimg/default/icon_weixin_no.png"" align=""absmiddle"" title=""{0}""/>{0}", MyCache.GetAccountAPIConfig("weixin", "name"));
      ...
      Response.Write(tp);          
             ...
             <div class="tips">灰色的说明您没有绑定第三方登录平台！</div>
           </div>
       </div>    
