...
.border .img img{border:1px solid #ccc;padding:1px;}
	...
	<div class="tabs">	
			<ul>
				<li<%if (string.IsNullOrEmpty(KSCMS.S("show"))) Response.Write (" class='puton'"); %>><a href="MyGroupBuyOrder.aspx">近一个月的订单(<%=DataFactory.GetOnlyValue("select count(1) From KS_ProOrder Where datediff(month,[InputTime],getdate())=0 and Business='"+(new TemporaryVar()).UserName+"'")  %>)</a></li>
				<li<%if (KSCMS.S("show")=="2") Response.Write (" class='puton'"); %>><a href="MyGroupBuyOrder.aspx?show=2">三个月内的订单(<%=DataFactory.GetOnlyValue("select count(1) From KS_ProOrder Where datediff(month,[InputTime],getdate())<=2 and Business='"+(new TemporaryVar()).UserName+"'")  %>)</a></li>
				<li<%if (KSCMS.S("show")=="-1") Response.Write (" class='puton'"); %>><a href="MyGroupBuyOrder.aspx?show=-1">所有的订单(<%=DataFactory.GetOnlyValue("select count(1) From KS_ProOrder Where Business='"+(new TemporaryVar()).UserName+"'")  %>)</a></li>
			</ul>
	</div>
         ...
         <ItemTemplate>
			...
			<tr class="tdbg">
             ...
             <%# GetOrderStatus(Convert.ToString(Eval("OrderID")),Utils.StrToInt(Eval("ConsumptionMode")),Utils.StrToInt(Eval("IsBusinessConfirm")),Convert.ToString(Eval("alipaytradestatus")), Utils.StrToInt(Eval("status")), Convert.ToInt16(Eval("DeliverStatus")), Utils.StrToDecimal(Convert.ToString(Eval("MoneyReceipt"))), Utils.StrToDecimal(Convert.ToString(Eval("OrderTotalPrice"))))%>
				...
				<a href="ShowMyOrder.aspx?id=<%# Eval("id") %>&from=business">订单详情</a>
