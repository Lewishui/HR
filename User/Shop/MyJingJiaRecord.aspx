      ...
      <table  cellspacing="1" cellpadding="3" class="border" width="100%" align="center" border="0">
	   <tr class="title">
        ...
        <tr class="tdbg">
         ...
         <td><asp:Label ID="LabPayMoney" runat="server"></asp:Label>元

             <span style="color:#999">提示：若支付金额小于0，将退回部分保证金。</span>
         </td>
        </tr>
        <tr class="tdbg">
        ...
        <div class="cz_btn" style="text-align:center;padding:20px;"　id="showDoButton" runat="server">
            <asp:HiddenField ID="HidAskId" runat="server" Value="0" />
            <span><asp:Button ID="BtnConfirm" Text="确认交易" OnClientClick="return(checkSubmitForm())" CssClass="button" runat="server" OnClick="BtnConfirm_Click" /></span>
            <span><asp:Button ID="BtnCancel" Text="拒绝交易" CssClass="button" runat="server" OnClientClick="return(confirm('温馨提示：拒绝交易，您将损失保证金,确认操作吗？'));" OnClick="BtnCancel_Click" /></span>
            <span><input type="button" class="button" value="取消返回" onclick="history.back()" /></span>
        </div>
   </asp:Panel>
</div>
</asp:Content>