    ...
    <script>
        ...
        function checkform() {
           ...
           if (countyid == '') {
            ...
            if (tel != '' && tel.length < 7) {
                KesionJS.Alert('固定电话格式不正确，请重新输入!','jQuery("#<%=this.TxtTel.ClientID %>").focus()');
                return false;
            }

            if (mobile != '') {
                if (ismobile(mobile) == false) {
                    KesionJS.Alert('手机号码不正确，请重新输入!','jQuery("#<%=this.TxtMobile.ClientID %>").focus()');
                    return false;
                }
            }
    ...
    <tr class="title1" style="display:none">
     ...
     <%=KSCMS.S("action") == "modify" ? "修改" : "添加"%>收货地址</td>
            ...
            <tr class="tdbg">
                ...
                <td class="righttd">
                  <asp:TextBox CssClass="textbox" ID="TxtMobile" runat="server" Width="120" MaxLength="30"></asp:TextBox> &nbsp;或固定电话 <asp:TextBox CssClass="textbox" ID="TxtTel" runat="server"  Width="120"></asp:TextBox>
