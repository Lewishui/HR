   ...
   <%if (upBySwf == 1)  //用swfupload 
  {
     %>
      ...
      <% if (!string.IsNullOrEmpty(module))  //XML配置上传
      {
       ...
       <%}else if (channelId==10001){ //会员注册时的上传字段 
      ...
      %>
		...
		<table cellspacing="0" cellspadding="0" border="0">
		 ...
		 <%
           ...
           if (isPic == 1)
      {%>
		  <td><label><input type="checkbox" name="DefaultUrl" id="DefaultUrl" onClick="SetDefaultUrl(this)" value="1"<%=CreateThumbs==1?" checked":string.Empty %>>生成缩略图</label>
		  
		   <%if (ConfigManage.GetConfig().WaterMarkStatus!=0){ %>     
               <label><input name="AddWaterFlag" type="checkbox" id="AddWaterFlag" onClick="SetAddWaterFlag(this)" value="1"<%=AddWaterMark==1?" checked":string.Empty %> >添加水印</label> 
              <%} %>
