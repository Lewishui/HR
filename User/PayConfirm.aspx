...
<script runat="server" type="text/C#">
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            #region  订单合并支付
            using (DataTable dt = DataFactory.GetDataTable("select * from ks_proorder where payorderid='" + orderId + "'"))
            {
                if (dt.Rows.Count <= 0)
                {
                ...
                else
                {
                    ...
                    {
                        if (Utils.StrToInt(dr["IsFrontPay"].ToString()) == 1)
                        {
                            vmoney += Utils.StrToDecimal(dr["FrontMoney"].ToString());
                        }
                        else
                        {
                            vmoney += Utils.StrToDecimal(dr["OrderTotalPrice"].ToString()) - Utils.StrToDecimal(dr["MoneyReceipt"].ToString());
                        }
                    }
        ...
        else  if (from == "shopgroupone")
        {
            #region  一元购支付
            //KSCMS.Die(orderId);
            
            string orderid = string.Empty;
            using (DataTable dt = DataFactory.GetDataTable("select * from ks_progrouponeorder where payorderid='" + orderId + "'"))
            {
                if (dt.Rows.Count <= 0)
                {
                    Utils.ShowError("找不到该订单，无法支付!", 1);
                ...
                else
                {
                    ...
                    foreach (DataRow dr in dt.Rows)
                    {
                        if (Utils.StrToInt(dr["Nowstatus"].ToString()) == 0)
                        {
                            vmoney += Utils.StrToDecimal(dr["OrderNowPrice"].ToString());
                        }
                        //num = Utils.StrToInt(dr["nownum"].ToString());
                        //orderid = dr["orderid"].ToString();
                        //DataFactory.ExecuteNonQuery("update KS_ProGroupOneOrder set nowstatus=1,OrderTotalPrice = OrderTotalPrice+" + Utils.StrToDecimal(dr["OrderNowPrice"].ToString()) +" where orderid='"+orderId+"'");
                        //int status = Utils.StrToInt(DataFactory.GetOnlyValue("select top 1 nowstatus from KS_ProGroupOneOrder where orderid='"+orderId+"'"));
                        //if (status==1)
                        //{
                        //    ShoppingCartUser.GetSerialNumber(num, orderId, orderid);
                        //}
                    }
            ...
            using (DataTable dt = DataFactory.GetDataTable("select top 1 * from ks_proorder where orderid='" + orderId + "'"))
            {
                ...
                else
                {
                    ...
                    else //最少支付订金
                    {
                        ...
                        if (string.IsNullOrEmpty(KSCMS.S("money")))
                        {
                            vmoney = Utils.StrToDecimal(dr["FrontMoney"].ToString());
                        }
                        else
        ...
        else if (from == "feepackages")  //购买收费套餐包
        {
            #region 购买收费套餐包
            CheckUserLogin.Run();
            int packId = Utils.StrToInt(KSCMS.S("id"));
            if (packId == 0)
            {
                Utils.ShowError("请选择一个要购买的套餐！", 1);
            }
            using (DataTable dt = DataFactory.GetDataTable("Select top 1 a.*,b.discount From KS_UserFeePackages a inner join KS_UserFeeDiscount b on a.id=b.packid Where a.status=1 and b.groupid=" + UserManage.GetUserInfo().GroupID + " and a.ID=" + packId))
            {
                if (dt.Rows.Count <= 0)
                {
                    Utils.ShowError("出错啦，找不到您要购买的套餐！", 1);
                }
                else
                {
                    DataRow dr = dt.Rows[0];
                    subject = "购买套餐包["+dr["title"].ToString()+"]费用";
                    decimal money = Utils.StrToDecimal(dr["price"].ToString());
                    decimal discount = Utils.StrToDecimal(dr["discount"].ToString());
                    if (discount == 0) discount = 1;
                    int days = Utils.StrToInt(dr["days"].ToString());
                    if (money == 0 || days == 0)
                    {
                        Utils.ShowError("对不起，系统配置有误，不能完成购买，请联系管理员！", 1);
                    }

                    vmoney = money * discount;
                    orderId = string.Concat("pack", DateTime.Now.ToString("yy"), DateTime.Now.ToString("MM"), DateTime.Now.ToString("dd"), DateTime.Now.ToString("HH"), DateTime.Now.ToString("mm"), DateTime.Now.ToString("ss"),"_"+packId);
                }
            }
            #endregion
        }
        ...
        {
            #region 自定义表单的支付
            using (DataTable dt = DataFactory.GetDataTable("Select top 1 * From KS_Form Where FormID=" + Utils.StrToInt(KSCMS.S("formID")) + ""))
            {
                if (dt.Rows.Count <= 0)
                {
                    Utils.ShowError("找不到该表单，无法支付!", 1);
                }
                else
                {
                    DataRow dr = dt.Rows[0];
                    subject = "支付["+dr["subject"].ToString()+"]费用";
                    using (DataTable dti = DataFactory.GetDataTable("select top 1 recordid From KS_FormRecord" + dr["formid"].ToString() + " Where ispay=1 and Recordid=" + Utils.StrToInt(KSCMS.S("orderid"))))
                    {
                        if (dti.Rows.Count > 0)
                        {
                            Utils.ShowError("该订单已支付过，不能再支付了!", 1);
                    ...
                    vmoney = Convert.ToDecimal(dr["paymoney"].ToString());
                    orderId = "form_" + dr["formid"].ToString() + "_" + KSCMS.S("orderid");
                }
            }
            #endregion
        }
        ...
        else if (string.IsNullOrEmpty(subject))
        {
            ...
            if (from != "recharge" && from != "feepackages")
            {
        ...
        string note = from + "∮" + payPlatformId;   //格式：  支付模块∮支付平台ID
        if (!DataFactoryUser.Exists("KS_UserRecord", "orderid='" + Utils.ChkSQL(v_oid) + "'"))
        {
            Kesion.APPCode.Public.addUserRecord(userName, 6, note, v_oid);
        }
        ...
        switch (payPlatformId)
        {
            ...
            case 8:   //支付宝即时到帐
                ...
                if (!Utils.IsMobileDevice())  //PC网站支付
                {
                ...
                if (Utils.IsMobileDevice())  //手机网站支付
                {
                    md5Builder.Append("&seller_id=" + partner);
                    md5Builder.Append("&service=alipay.wap.create.direct.pay.by.user");
                }else
                {
                ...
                if (Utils.IsMobileDevice())  //手机网站支付
                {
                    md5Builder.Append("&total_fee=" + v_amount);
                }
                md5Builder.Append(payOnlineKey);
                ...
                if (Utils.IsMobileDevice())  //手机网站支付
                {
                    strHiddenField.Append("<input type='hidden' name='seller_id' value='" + partner + "'>\n");
                    strHiddenField.Append("<input type='hidden' name='service' value='alipay.wap.create.direct.pay.by.user'>\n");
                }
                else
                {
                ...
                if (Utils.IsMobileDevice())  //手机网站支付
                {
                    strHiddenField.Append("<input type='hidden' name='total_fee' value='" + v_amount + "'>\n");
                }
            ...
            case 14:   //银联在线支付
                ...
                v_urlBuilder.Append("User/PayOnline/PayResultUnionPay.aspx");
                v_url = v_urlBuilder.ToString();
                ...
                param["frontUrl"] = v_ShowResultUrl;    //前台通知地址      
                param["backUrl"]= v_url;  //后台通知地址，改自己的外网地址
                param["signMethod"]= "01";//签名方法
                param["channelType"] = "08";//渠道类型，07-PC，08-手机
                param["accessType"] = "0";//接入类型
                param["merId"] = AccountID.Trim();//商户号，请改成自己的商户号
                param["orderId"] = v_oid.Trim();//商户订单号，可任意修改
                param["txnTime"] = DateTime.Now.ToString("yyyyMMddHHmmss");//订单发送时间
                param["txnAmt"] = (Utils.StrToDecimal(v_amount)*100).ToString("f0");//交易金额，单位分
                ...
                Response.Write(html);
                KSCMS.Die("正在转向支付页面,请稍候...");



                break;
            case 15:  //微信支付
                if (new TemporaryVar().FromPage == "3g" || Utils.IsMobileDevice())
                {
                    Response.Redirect("payonline/wx/send.aspx?orderid=" + v_oid + "&flag=" + from + "&uid=" + new TemporaryVar().UserID);
                }
                else
                {
                    Response.Redirect("payonline.aspx?from=wx&flag=" + from + "&money=" + Request.QueryString["Money"] + "&orderid=" + v_oid);
                }
                break;
            default:
        ...
        if (payPlatformId==8 && Utils.IsAppDevice())
        {

            m_HiddenValue = @"<!DOCTYPE html>
            <html>
            <head> 
            <title>在线充值</title>
            ...
            <meta name=""MobileOptimized"" content=""320"">
             <script src='http://static.ydbimg.com/Scripts/jquery-1.9.1.js' type='text/javascript'></sc"+@"ript>
            <script src='http://static.ydbimg.com/API/YdbOnline.js' type='text/javascript'></sc"+@"ript>
          <body>
               <div style=""margin-top:100px;font-size:26px"">正在转向支付页面,请稍候...</div>
             ";
            m_HiddenValue += @"<script>
            ...
            m_HiddenValue += string.Format(@"YDB.SetAlipayInfo('{0}', '{1}', '{2}','{3}');",subject,subject,vmoney,v_oid);
            m_HiddenValue += @"}); </s"+@"cript>";
