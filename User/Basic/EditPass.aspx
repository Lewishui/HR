...
<script>
    function checkform() {
       ...
       if ($("#<%=this.PassWord.ClientID %>").val().length<6) {
            KesionJS.Alert('新密码长度不能少于六位！', "$('#<%=this.PassWord.ClientID %>').focus()");
            return false;
       }
       if ($("#<%=this.PassWord.ClientID %>").val() == $("#<%=this.OldPass.ClientID %>").val()) {
            KesionJS.Alert('新密码和旧密码不能设置一样！', "$('#<%=this.PassWord.ClientID %>').focus()");
            return false;
        }
	...
	<div class="r_tybox nomar">
        ...
        <div class="clear"></div>
        <div class="r_padbox">
        	<div class="padtitle">修改密码</div>
 		<table  cellspacing="1" cellpadding="3" class="border" width="100%" align="center" border="0">

                          <%if (__info.IsApi == 1)
                            { %>
                            <tr class="tdbg">
                            <td colspan="2" style="font-size:14px;color:green;height:80px; text-align:left;line-height:30px;padding:10px;">温馨提示：您是通过第三方网站账号登录本站的，您可以设置新密码，下次就可以直接使用用户名 <span style="color:Red"><%=__info.UserName %></span> 和以下设置的密码登录本站了。</td>
                          </tr>
                          <%}
                            else
                            { %>
                          <tr class="tdbg">
                            ...
                            <td style="text-align:left;"><asp:TextBox ID="OldPass" CssClass="textbox"  TextMode="password" runat="server"></asp:TextBox><span class="tips">您的旧登录密码，必须正确填写。</span><span style="color: red">*<asp:RequiredFieldValidator
                          ...
                          <tr class="tdbg">
                            ...
                            <td style="text-align:left;"><asp:TextBox ID="PassWord" CssClass="textbox" TextMode="password" runat="server"></asp:TextBox><span class="tips">请输入您的新密码！</span>
                          ...
                          <tr class="tdbg">
                            ...
                            <td style="text-align:left;"><asp:TextBox ID="RePassWord" CssClass="textbox" TextMode="password" runat="server"></asp:TextBox><span class="tips">同上。</span>
                              <span style="color: red">* 
                          ...
                          <tr class="tdbg">
                            ...
                            <td style="text-align:left;border-bottom:0px;">
                                <div class="cz_btn">
                                      <span><asp:Button OnClientClick="return(checkform());" ID="Button1" CssClass="button" runat="server" OnClick="Button1_Click" Text="OK!确定修改" /></span>
                                       
                                      <span><input type="reset" class="button" value=" 重 置 " /></span>
                                </div>
              				</td>    
                           </tr>                 
            </table>
