  ...
  <tr class="title">
   ...
   <td  height="22" align="center">
        <% if (new TemporaryVar().FromPage == "3g")
            { %>
       <a href="/<%=MyCache.GetCacheConfig(73) %>/user/index.aspx">进入会员中心</a> | <a href="/<%=MyCache.GetCacheConfig(73) %>/index.aspx">返回首页</a>
       <%}
    else
    { %>
       <a href="../index.aspx">进入会员中心</a> | <a href="../../">返回首页</a>
       <%} %>
   </td>
  </tr>
</table>
...
private void UnionPay(){
    ...
    if (Request.HttpMethod == "POST")
    {
        ...
        if (SDKUtil.Validate(resData, Encoding.UTF8))
        {
            ...
            {
                string orderTipsTp = Kesion.APPCode.OrderProcess.DoPayOnlineTips(order_no,true); //订单付款成功提醒管理员
    ...
    if (resHandler.isTenpaySign())
    {
        ...
        {       //即时到账 
            if ("0".Equals(trade_state))
            {
                ...
                if (!string.IsNullOrEmpty(KSCMS.S("from")))
				{
					string orderTipsTp = Kesion.APPCode.OrderProcess.DoPayOnlineTips(out_trade_no,true); //订单付款成功提醒管理员
					message.Append(orderTipsTp);
				}
                else
                {
 ...
 private void ChainBank(){ //网银
    ...
    string md5string = Utils.MD5(v_oid + v_pstatus + v_amount + v_moneytype + PayOnlineKey);
        ...
        if (md5string.ToUpper() == v_md5.ToUpper() && v_pstatus == "20")
        {
            ...
            if (!string.IsNullOrEmpty(KSCMS.S("from")))
            {
                string orderTipsTp = Kesion.APPCode.OrderProcess.DoPayOnlineTips(v_oid,true); //订单付款成功提醒管理员
                message.Append(orderTipsTp);
            }
            else
            {
