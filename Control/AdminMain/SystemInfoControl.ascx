...
<br />
厦门科汛软件有限公司 <span style="font-size:18px;">&copy;</span> 版权所有 <Br />
                       ...
                       jQuery.ajax({
                           ...
                           success: function (d) {
                               switch (d) {
                                   ...
                                   case 'true': //有可用的升级补丁
                                       if (autoCheck == 0) {
                                           $.dialog.alert("系统检测到有可用的升级版本！", function () {
                                               parent.openWin("升级提示：", "<%=KSCMS.GetInstallDir()+MyCache.GetCacheConfig(4)%>/System/KS.Update.aspx?action=showupdateinfo&rnd=" + Math.random(), false, 850, 350);
                                           }, function () {
