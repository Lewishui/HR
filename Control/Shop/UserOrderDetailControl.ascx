...
.cz_btn span{ margin:0;}
.cz_btn span input{ margin-right:10px;}
</style>
...
<table border="0" cellpadding="2" cellspacing="1" class="border" width="98%" align="center">
    <tr class="title1">
        <td>
         ...
         订购时间：<asp:Label ID="LabInputTime" runat="server" Text="Label"></asp:Label>
      ...
      <%} %>
        <%if (dr["ordertype"].ToString() == "1" && !string.IsNullOrEmpty(dr["verifycode"].ToString()) && dr["business"].ToString() != (new TemporaryVar()).UserName)
         ...
         <%if (Utils.StrToFloat(dr["PayDiscountMoney"].ToString())>0){ %>
          (优惠：<span style="color:green"><%=MyCache.GetCurrencySymbol%><asp:Label ID="LabPayDiscount" runat="server" Text="Label"></asp:Label></span> <%=MyCache.GetCurrencyUnit%>)
         <%} %><br />
    ...
    <td class="splittd">		
        ...
        <asp:GridView ID="GVGroupBuy" style="border-bottom:none" runat="server" AutoGenerateColumns="False" CssClass="border" EmptyDataText="没有数据!"  Width="99%" CellPadding="1" CellSpacing="1" GridLines="None" HorizontalAlign="Center">
         <Columns>
              ...
              <asp:TemplateField HeaderText="图片">
                ...
                <itemtemplate>
                    <img onerror="this.src='../../sysimg/nopic.gif';" src='<%# string.IsNullOrEmpty(Convert.ToString(Eval("DefaultPic")))?MyCache.GetCacheShopConfig(9):Eval("DefaultPic")  %>' width="60" height="45" style="border:1px solid #efefef;padding:1px" />
                </itemtemplate>
              </asp:TemplateField>
              <asp:TemplateField HeaderText="商品名称">
                <itemstyle horizontalalign="Left" Width="400px" />
                <itemtemplate>
                 <a href="../../shop/<%=strType %>?Id=<%# Eval("id") %>" target="_blank"><%# Eval("subject").ToString() %></a>
                </itemtemplate>
              </asp:TemplateField>
        ...
        <span style="color:green"><asp:Label ID="LabCouponTips" runat="server"></asp:Label>
            <asp:Label ID="LabRedPacketTips" runat="server"></asp:Label>
        </span>
 ...
 <br /><br />
        ...
        <div style="margin:10px;text-align:center;" class="noprint">
            ...
            <%if (KSCMS.S("from").Equals("business")) { %>
                <span><input type="button" class="button" id="backBtn" value="返回订单管理" onclick="history.back(-1);" /></span>
            <%} else { %>
             <%if (dr["status"].ToString()!="3") { %>
            <span><input type="button" class="button" id="yfkBtn" runat="server" value="订单支付"/></span>
            <span><input type="button" class="button" id="backBtn" runat="server" value="退货退款"/></span>
            <span><input type="button" class="button" id="signBtn" runat="server" value="确认收货"/></span>
            <%} %>
            <span><input type="button" value="打印订单" class="button" onclick="window.open('MyOrderPrint.Aspx?id=<%=id %>');" /></span>
            <span><input type="button" class="button" id="delBtn" runat="server" value="删除订单"/></span>
            <%if (KSCMS.GetComeUrl().ToLower().IndexOf("mainurl")!=-1){ %>
            <span><input type="button" class="button" id="Button1" value="查看近一个月订单" onclick="location.href='myorder.aspx';" /></span>
            <%}else{ %>
            <span><input type="button" class="button" id="backBtn" value="返回订单管理" onclick="history.back(-1);" /></span>
            <%} 
              }%>
        ...
        <script type="text/javascript">
           ...
           jQuery("#<%=this.backBtn.ClientID %>").click(function() {
               ...
               jQuery("#<%=this.signBtn.ClientID %>").bind("click", function() {
                   KesionJS.Confirm("此操作不可逆，您确定收到商品了吗？", 'location.href="shop/showmyorder.aspx?id=' + id + '&action=signorder";', null);
               });

               jQuery("#<%=this.delBtn.ClientID %>").bind("click", function() {
                   KesionJS.Confirm("确定删除该订单吗？", 'location.href="shop/myorder.aspx?id=' + id + '&action=delorder";', null);
               });
           });
   ...
   <asp:Repeater ID="Repeater1" runat="server">
    ...
    <ItemTemplate>
       <tr style="display:<%# bool.Parse((Repeater1.Items.Count>0).ToString())?"":"none"%>"><td colspan="5"><hr /></td></tr>

      <tr class="tdbg">
          <td class="splittd" width="120">
           <strong>申请时间：</strong>
          </td>
      ...
      <tr class="tdbg">
          ...
          <td  class="splittd resonImg"><%# Eval("remark") %></td>
      </tr> 
      <tr class="tdbg">
          <td class="splittd" >
           <strong>货款退回方式：</strong>
          </td>
      ...
      <tbody<%# Convert.ToInt16(Eval("MoneyReturnType"))==2?"":" style='display:none'" %>>
          <tr class="tdbg">
              ...
              <td  class="splittd">开户银行名称：<%# Eval("MoneyReturnBankName")%>
                  <br />银行卡号：<%# Eval("MoneyReturnBankCard")%>
                  <br />银行卡户名：<%# Eval("MoneyReturnCardName")%>
              </td>
          </tr> 
        ...
        <tbody<%# Convert.ToInt16(Eval("status"))==1?"":" style='display:none'" %>>
          ...
          <tr class="tdbg">
              ...
              <td  class="splittd" style="color:green;"><%# Convert.ToInt16(Eval("HandlerStatus"))==3?"同意换货":(Convert.ToInt16(Eval("HandlerStatus"))==1?"同意":"妥协交易")%></td>
          </tr> 
        </tbody>
     </ItemTemplate>
     <FooterTemplate>
