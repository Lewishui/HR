...
<div class="content-area">
   ...
   $(document).ready(function(){
     showSpecified();
     showInvoice();
     $("#<%=this.DrpDeliveryDate.ClientID %>").bind("change",function(){
     showSpecified();
     });
       showStore();
       $("#<%=this.DrpDeliveryType.ClientID%>").bind("change",function(){
           showStore();
       });
   });
   function showStore(){
       if (parseInt($("#<%=this.DrpDeliveryType.ClientID%> option:selected").attr("iszt"))==1){
         //  $("#showStore").show();
       }else{
         //  $("#showStore").hide();
       }
...
<table border="0" cellpadding="2" cellspacing="1" class="CTable" width="99%">
 ...
 <asp:Panel ID="Address2" runat="server">
     <table cellSpacing=1 cellPadding=2 width="100%" align=center border=0 class="CTable sTable">
    <tr class=tdbg>
    ...
    <tr class=tdbg vAlign=top>
      ...
      <td width="38%" style="text-align: left">
         ...
         <tr class="tdbg" valign="top">
             ...
             <td style="text-align: left" width="38%"> &nbsp;<asp:TextBox  CssClass="textbox" ID="TxtEmail" runat="server"></asp:TextBox></td>
         </tr>
    <tr class=tdbg>
    ...
    <tr class=tdbg>
      ...
      <td width="38%" style="text-align: left">
          ...
          <div class='message'  id="showStore">
          <%
             // if (Utils.StrToInt(dr["StoreID"].ToString()) >0)
              {
                  Response.Write("<ul><strong>配货点：</strong><br/>");
                  using (DataTable dt = MyCache.GetCacheProStoreTable())
                  {
                      if (dt.Rows.Count > 0)
                      {
                          ...
                          {
                              if (Utils.StrToInt(dr["StoreID"].ToString())==Utils.StrToInt(drr["id"].ToString())){
                                Response.Write("<li><input checked type='radio' name='storeid' value='" + drr["id"].ToString() + "'>" + drr["title"].ToString() + "&nbsp;&nbsp;地址："+dr["address"].ToString()+"</li>");

                              }else{
                                 Response.Write("<li><input type='radio' name='storeid' value='" + drr["id"].ToString() + "'>" + drr["title"].ToString() + "&nbsp;&nbsp;地址："+dr["address"].ToString()+"</li>");
                              }
                          }
                      }
                  }
    ...
    <tr class=tdbg vAlign=top>
      ...
      <td width="38%" style="height: 56px; text-align: left;">
          ...
          <td align=right width="12%" style="height: 56px">订单附言：</td>
      <td width="38%" style="height: 56px; text-align: left">
         ...
         <tr class="tdbg" valign="top">
             ...
             <div id="showSpecifiedDate">
               <asp:TextBox  CssClass="textbox Wdate" ID="TxtSpecifiedDate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd'});" runat="server"></asp:TextBox>
             </div></td>
             <td align="right" width="12%">
                 送货时间：</td>
             <td width="38%" style="text-align: left">
       ...
       <asp:GridView ID="GVGroupBuy" style="border-bottom:none" runat="server" AutoGenerateColumns="False" CssClass="CTable" EmptyDataText="没有数据!"  Width="99%" CellPadding="1" CellSpacing="1" GridLines="None" HorizontalAlign="Center">
         ...
         <Columns>
              ...
              <asp:TemplateField HeaderText="商品名称">
                ...
                <itemtemplate>
                   ...
                   <asp:HiddenField ID="hidScore" runat="server" Value='<%# Eval("Score") %>' />
                    <%# getProOrder(Utils.StrToInt(Convert.ToString(Eval("ClassID"))), Convert.ToString(Eval("FileName")), Convert.ToString(Eval("TurnUrl")), Convert.ToInt32(Eval("ItemID")), Utils.StrToInt(Convert.ToString(Eval("ChannelID"))), Utils.StrToInt(Convert.ToString(Eval("InfoID"))), string.IsNullOrEmpty(Convert.ToString(Eval("ITitle"))) ? Convert.ToString(Eval("Title")) : Convert.ToString(Eval("ITitle")), Convert.ToInt32(Eval("isBundleSale")), Convert.ToInt32(Eval("isChangeBuy")), Convert.ToInt32(Eval("isLimitBuy")),Convert.ToString(Eval("AttributeCart")), Utils.StrToInt(Convert.ToString(Eval("Attrid"))))%>
                </itemtemplate>
              </asp:TemplateField>
              ...
              <asp:TemplateField HeaderText="单位">
                ...
                <itemtemplate>
                   <%# Convert.ToInt32(Eval("isBundleSale"))==1?"--":Eval("Unit") %>
                </itemtemplate>
              </asp:TemplateField>
              ...
              <asp:TemplateField HeaderText="单件重量"  Visible="false">
                <itemstyle width="55px" horizontalalign="Center" />
                <itemtemplate>
                   <%# Convert.ToInt32(Eval("isBundleSale")) == 1 ? "--" : string.Format("{0:G2} 公斤", Convert.ToString(Eval("Weight")))%>
                </itemtemplate>
              </asp:TemplateField>
              <asp:TemplateField HeaderText="单件积分"  Visible="false">
                <itemstyle width="55px" horizontalalign="Center" />
                <itemtemplate>
                   <%# Convert.ToInt32(Eval("isBundleSale")) == 1 ? "--" : string.Format("{0:G2} 分", Convert.ToString(Eval("Weight")))%>
                </itemtemplate>
              </asp:TemplateField>
    ...
    <div class="main-button">
        ...
        <%if (orderType != 1)
          { %>
         <input type="button" class="button" value="添加商品" class="button" onclick='parent.openWin("添加商品至订单","<%=KSCMS.GetInstallDir()+MyCache.GetCacheConfig(4)%>/Shop/KS.ShopAddToOrder.aspx?action=addtoorder&id=<%=id %>",true,800,420);' />
         <%} %>
