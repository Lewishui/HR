...
<table class="Border" cellSpacing=1 cellPadding=2 width="99%" align="center" border=0>
    ...
    <tr class=tdbg>
      <td style="text-align:center;width:350px;line-height:38px;">
              <script>
   ...
   <input onclick="UnSelectAll('<%=this.LstField.ClientID %>    ')" type="button" value="取消选定字段" class="button" name="Submit" />
      ...
      <td valign="top" style="line-height:36px;">
          <strong>订单类型：</strong>
       ...
       <div id="showdate">
       <strong>定购日期：</strong>从<asp:TextBox onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" ID="TxtBeginDate" CssClass="textbox Wdate" runat="server"></asp:TextBox>至
       <asp:TextBox onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'});" ID="TxtEndDate" CssClass="textbox Wdate" runat="server" />
      </div>
        <div>
         <strong>付款状态：</strong>
            <asp:RadioButtonList ID="RdbPayFlag" runat="server" RepeatDirection="Horizontal" RepeatLayout="Flow">
    ...
    <asp:TextBox  style="color:#ccc" Text="" CssClass="textbox" ID="TxtExportKey" runat="server"></asp:TextBox>
     ...
     <br />
          <strong>导出类型：</strong>
          <asp:RadioButtonList ID="RdbExportType" runat="server" RepeatDirection="Horizontal" RepeatLayout="Flow">
              <asp:ListItem Value="0" Selected="True">Excel（xls）</asp:ListItem>
              <asp:ListItem Value="1">Word（doc）</asp:ListItem>
          </asp:RadioButtonList>
      </td>
  ...
  <div class="main-button">
		...
		<Input class=button onclick="history.back();" type=button value=取消返回 name=Submit>
  </div>
</div>  
