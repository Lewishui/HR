...
<asp:Content ID="Content1" ContentPlaceHolderID="KSContent" Runat="Server">
     ...
     <div id="manage_top" class="menu_top_fixed">
       ...
       <ul>
         ...
         <li id='p5'><asp:LinkButton ID="attribbutton" runat="server" OnClick="attribbutton_Click"><b></b>批量设置</asp:LinkButton></li>
         ...
         <li id='p10'><asp:LinkButton ToolTip="一键修正栏目排序等错误" ID="LinkButton4" runat="server" OnClick="LinkButton4_Click"><b></b>修正排序</asp:LinkButton></li>

            <%if (from!="shop"){ %>
         <li id='p15'><a href="javascript:;" onclick="refreshClass()"><b></b>生成HTML</a></li>
            <%} %>
...
{       
  ...
  jQuery('#C'+ID).attr('class','tree_Close');
          ...
          <ItemTemplate>
            ...
            <tr id='sub<%#Eval("classid") %>' onmouseover="jQuery('#a<%# Eval("classid") %>').show();jQuery(this).addClass('CtrMouseOver');jQuery(this).removeClass('CtrMouseOut');" onmouseout="jQuery('#a<%# Eval("classid") %>').hide(); jQuery(this).addClass('CtrMouseOut');jQuery(this).removeClass('CtrMouseOver');" onclick="chk(this)">
             ...
             <%if (KSCMS.S("flag") != "showall")
               ...
               { %><%# getClassName(Convert.ToInt32(Eval("ClassID")),Eval("Classname", "{0}"), Convert.ToInt16(Eval("child")), Utils.StrToInt(Convert.ToString(Eval("NextID"))), Convert.ToInt16(Eval("depth", "{0}")), Convert.ToInt16(Eval("ClassType", "{0}")))%><%} %>
           ...
           <%} %>
             <td class="Ctd link-operate" align="center">
                 <%if (KSCMS.S("from").Equals("shop"))
                     { %>
                 <a title="管理商品" class="link-color" href="../shop/KS.ShopManage.Aspx?channelid=<%# Eval("channelid") %>&classid=<%# Eval("classid") %>"><%# Eval("infonum")%> <%# MyCache.GetCacheModel(Utils.StrToInt(Convert.ToString(Eval("channelid"))),6) %></a>
                 <%}
                  else
                    { %>
                  <a title="管理文档" href="KS.ContentManage.Aspx?channelid=<%# Eval("channelid") %>&classid=<%# Eval("classid") %>"><%# Eval("infonum")%> <%# MyCache.GetCacheModel(Utils.StrToInt(Convert.ToString(Eval("channelid"))),6) %></a>
        ...
        <!--
              ...
              jQuery.get("../../plus/ajaxs.ashx",{a:"modelename",modelid: modelid},function(val){
                  ...
                  jQuery.get("../../plus/ajaxs.ashx", { col: 3, a: "modelename", modelid: modelid }, function (val) {
                      if (val != '') {
                          ...
                          jQuery('#<%=this.TxtClass3GTemplate.ClientID %>').val('{$TemplateDir}' + val + '/栏目页.html');
                          jQuery('#<%=this.TxtContent3GTemplate.ClientID %>').val('{$TemplateDir}' + val + '/内容页.html');
                      }
                  });
              });
            }
	      ...
	      if (classtype==1){
            ...
            function showHtmlFlag(){
                var v=$("#<%=this.CreateHtmlFlag.ClientID%> :radio[checked]").val()
                if (v == "1") {
                    $("#showAutoFsoOption").hide();
                    $("#showFsoClass").hide();
                    $("#showFsoContent").hide();
                } else if (v == "2"){
                    $("#showAutoFsoOption").show();
                    $("#showFsoClass").hide();
                    $("#showFsoContent").show();
                }else {
                    $("#showAutoFsoOption").show();
                  ...
                  <tr id="ModelArea">
                       <td align="right" class="lefttd" style="height: 37px;"><strong> 绑定模型：</strong></td>
                         <td class="righttd" style="height: 37px">
                             ...
                             <span id="Span1" runat="server"><a href="../Model/KS.Model.aspx" class="link-color">管理系统模型</a></span>
                       </td>
                     </tr>
                  ...
                  <tr>
                    ...
                    <td class="righttd">
                        <table cellspacing="0" cellpadding="0"><tr><td><asp:TextBox CssClass="textbox" ID="ClassImg" runat="server" Width="480px"></asp:TextBox>&nbsp;<input type="button" class="button" onclick="SelectFiles('<%=this.ClassImg.ClientID %>',0);" value="选择图片"></td>
                            ...
                            <td style="padding-left:4px;padding-top:12px;"> <%=Kesion.APPCode.UploadAPI.EchoUpload("classpic",this.ClassImg.ClientID,150) %></td>
                            <%} %>
                            <td><img id="show<%=ClassImg.ClientID %>" src="<%=classImgSrc %>" width="60" height="60" style="border:1px solid #f1f1f1;" onerror="this.src='../../sysimg/nopic.gif'"/></td>
                          </tr></table>
         ...
         <tbody id="ClassTemplateArea">
              <tr>
                ...
                <td class="righttd" style="height: 30px">
                    ...
                    <input type="button" class="button" onmouseover="this.className='button_on';" onmouseout="this.className='button';" value="选择模板..." onClick="SelectTemplate('<%=this.ClassTemplate.ClientID %>',0);"/></td>
            </tr>
              <tr<%=MyCache.GetCacheConfig(72)=="0"?" style='display:none'":"" %>>
                ...
                <td class="righttd" style="height: 30px">
                    ...
                    <input class="button" onClick="SelectTemplate('<%=this.TxtClass3GTemplate.ClientID %>',1);" 
                        onmouseout="this.className='button';" onmouseover="this.className='button_on';" type="button" value="选择模板..." /></td>
              </tr>
            </tbody>
           <tbody id="ContentTemplateArea">
            <tr>
                ...
                <td class="righttd" style="height: 30px">
                    ...
                    <input type="button"  class="button" onmouseover="this.className='button_on';" onmouseout="this.className='button';" value="选择模板..." onClick="SelectTemplate('<%=this.ContentTemplate.ClientID %>',0);"/>
               ...
               <tr<%=MyCache.GetCacheConfig(72)=="0"?" style='display:none'":"" %>>
                ...
                <td class="righttd" style="height: 30px">
                    <asp:TextBox ID="TxtContent3GTemplate" runat="server" CssClass="textbox" 
                        Width="480px"></asp:TextBox>
                    <input class="button" 
                        onClick="SelectTemplate('<%=this.TxtContent3GTemplate.ClientID %>',1);" 
			...
			<script type="text/javascript">
              ...
              <table width="100%" border="0" cellpadding="1" cellspacing="1" class="CTable">
                  ...
                  <tr id="domainarea" runat="server">
                      <td align="right" class="lefttd" height="30" style="width: 227px">
                          <strong>绑定域名： </strong></td>
                      <td class="righttd">
                          <strong>子 域 名：</strong><asp:TextBox CssClass="textbox" ID="ClassDomain" runat="server" Width="378px"></asp:TextBox>
                          <br /><span class="tips">如：http://news.kesion.com，后面不能加"/"，此功能只对一级栏目有效。</span>
                          <br />
                          <strong>绑定方式：</strong><asp:RadioButtonList ID="RdbDomainBindType" runat="server" RepeatDirection="Horizontal" RepeatLayout="Flow">
                              <asp:ListItem Value="0" Selected="True">当前站点直接绑定</asp:ListItem>
                              <asp:ListItem Value="1">新建站点绑定到生成的HTML目录</asp:ListItem>
                               </asp:RadioButtonList>
                      </td>
                  </tr>
                  <tr>
		  ...
		  <h2 class="tab">生成选项</h2>
            ...
            <table width="100%" border="0" cellpadding="1" cellspacing="1" class="CTable">
              ...
              <tr id="showAutoFsoOption">
                ...
                <tr id="showFsoClass">
                    <td align="right" class="lefttd" style="width: 199px">
                        <strong>栏目生成配置：</strong></td>
                    <td class="righttd">
                         <table border="0">
                             ...
                             <tr>
                                 <td align="center"><strong>生成路径规则：</strong></td>
                                 <td><asp:RadioButtonList runat="server" ID="RdbClassFsoDirRule" RepeatLayout="Flow">
                            <asp:ListItem Value="0" Selected="True">按目录级别顺序结构生成列表页(<span style='color:red'>Tips:启用绑定域名，且采用“新建站点绑定到生成的HTML目录”时，只能选择此项</span>)<br />
                                <span style="color:green">
                                如：第1页为：/总生成目录/aaa/bbb/ccc/index.html<br />
                                &nbsp;&nbsp;&nbsp;&nbsp;第2页为：/总生成目录/aaa/bbb/ccc/index_2.html </span>
                            </asp:ListItem>
                            <asp:ListItem Value="1">本栏目按"自定义前缀+栏目ID"生成在模型的总生成目录下面<br />
                                <span style="color:green">
                                如：栏目ID号为100则生成如下：<br />
                                   &nbsp;&nbsp;&nbsp;&nbsp;第1页为：/总生成目录/自定义前缀_100.html<br />
                                   &nbsp;&nbsp;&nbsp;&nbsp;第2页为：/总生成目录/自定义前缀_100_2.html 
                              </span>
                            </asp:ListItem>
                            <asp:ListItem Value="2">本栏目生成在一级栏目（频道）英文目录下,按如下规则生成<br />
                                <span style="color:green">
                               如：一级栏目 "教育频道" 英文名称："edu",那么生成如下：<br />
                               &nbsp;&nbsp;&nbsp;&nbsp;第1页为：/总生成目录/edu/index.html<br />
                               &nbsp;&nbsp;&nbsp;&nbsp;第2页为：/总生成目录/edu/index_2.html <br />
                              &nbsp;&nbsp;&nbsp;&nbsp;如果是二级及以上的栏目(即"教育频道")下的栏目,如栏目ID号为101则生成如下：<br />
                               &nbsp;&nbsp;&nbsp;&nbsp;第1页为：/总生成目录/edu/自定义前缀_101.html<br />
                               &nbsp;&nbsp;&nbsp;&nbsp;第2页为：/总生成目录/edu/自定义前缀_101_2.html  </span>
                            </asp:ListItem>
                            <asp:ListItem Value="3">本栏目页"自定义前缀"生成在模型总生成目录下面<br />
                                <span style="color:green">
                                如：第1页为：/总生成目录/自定义前缀.html<br />
                               &nbsp;&nbsp;&nbsp;&nbsp;第2页为：/总生成目录/自定义前缀_2.html </span>
                            </asp:ListItem>
                        </asp:RadioButtonList></td>
                             </tr>
                             <tr>
                                 <td align="center"><strong>生成的栏目自定义前缀：</strong></td>
                                 <td>
                                    <asp:TextBox CssClass="textbox" ID="TxtClassFsoPre" runat="server" Text="{$ClassEname}" Width="283px"></asp:TextBox><br />
                                     <span class="tips">可用标签：{$ClassEname}-本栏目英文名 {$ClassID}-本栏目ID  {$TopClassEname}-一级栏目英文名 </span>
                                 </td>
                             </tr>
                         </table>
                ...
                <tr id="showFsoContent">
                    <td align="right" class="lefttd" style="width: 199px">
                        <strong>内容页生成配置：</strong></td>
                    <td class="righttd">
                         <table>
                             <tr>
                                <td align="right" class="lefttd" height="30" style="width: 199px">
                                ...
                                <td class="righttd" height="28">
                                    <asp:TextBox CssClass="textbox" ID="CreateInfoPath" runat="server" Text="{$ClassDir}/" Width="283px"></asp:TextBox>必须与＂/＂结束
                                    <br />
                                    <br />
                                    <select name='SelectInfoFsoPath' onchange="if (this.value!=''){jQuery('#<%=this.CreateInfoPath.ClientID %>').val(this.value);}">
                                        ...
                                        <option value='List/{$Month}/'>List/{$Month}/</option></select>
                                    <br />
  <span style='color:red'>Tips:启用绑定域名，且采用“新建站点绑定到生成的HTML目录”时，只能填写"{$ClassDir}/"。</span>
                            ...
                            <tr>
                                ...
                                <td class="righttd" height="28">
                                    <asp:TextBox CssClass="textbox" ID="CretaeInfoFname" runat="server" Text="{$InfoID}.html" Width="283px"></asp:TextBox>
                                    <br />
                                    <br />
        ...
        <%}else{ %>
		 ...
		 <%} %>
          ...
          <table width="100%"  border="0" cellpadding="1" cellspacing="1" class="CTable">
              <tr>
                  ...
                  <td class="righttd">
                      <asp:RadioButtonList ID="ClassPurview" runat="server">
                          <asp:ListItem Selected="True" Value="0">开放栏目  &lt;font color=red&gt;任何人（包括游客）可以浏览和查看此栏目下的文档。&lt;/font&gt;</asp:ListItem>
                  ...
                  <tr>
                      <td align="right" class="lefttd" height="30" style="width: 227px">
                          <strong>允许查看此栏目下文档的会员组：</strong>
                          <br />
                          <font color=blue>如果栏目是“认证栏目”，请在此设置允许查看此栏目下文档的会员组,如果在信息中设置了查看权限，则以文档中的权限设置优先</font>
                          </td>
              ...
              <tr>
                  <td align="right" class="lefttd" height="30" style="width: 227px">
                      <strong>默认阅读文档所需点数：<br />
                      </strong><span style="color: #0000ff">如果在文档中设置了阅读点数，则以文档中的点数设置优先</span></td>
                  <td class="righttd">
                      &nbsp;<asp:TextBox CssClass="textbox" ID="DefaultReadPoint" Text="0" style="text-align:center" runat="server" Width="55px"></asp:TextBox>
                      <span class="tips">免费阅读请设为 "<font color="red">0</font>"，否则有权限的会员阅读该栏目下的文档时将消耗相应点数，游客将无法阅读。</span></td>
              </tr>
              <tr>
                  <td align="right" class="lefttd" height="30" style="width: 227px">
                      <strong>默认阅读文档重复收费：<br />
                      </strong><font color="blue">如果在文档中设置了此选项，则以文档中的设置优先</font></td>
                  <td class="righttd">
                      <asp:RadioButtonList ID="DefaultChargeType" runat="server" CssClass="sTable">
                          <asp:ListItem Value="0" Selected=True>不重复收费(如果文档需扣点数才能查看，建议使用)</asp:ListItem>
                          ...
                          <asp:ListItem Value="2">会员重复阅文档&lt;input class=textbox type=&quot;text&quot; style=&quot;width:50px;text-align:center;&quot; value=&quot;10&quot; name=&quot;DefaultReadTimes&quot; size=&quot;5&quot;&gt;   页次后重新收费</asp:ListItem>
                          <asp:ListItem Value="3">上述两者都满足时重新收费</asp:ListItem>
                          <asp:ListItem Value="4">上述两者任一个满足时就重新收费</asp:ListItem>
                          <asp:ListItem Value="5">每阅读一页次就重复收费一次（建议不要使用,多页文档将扣多次点数）</asp:ListItem>
                      </asp:RadioButtonList></td>
              </tr>
              <tr>
                  <td align="right" class="lefttd" height="30" style="width: 227px">
                  ...
                  <td class="righttd">
                      ...
                      %  <span class="tips">会员在此栏目下添加文档时，该文档默认的分成比例</span></td>
              </tr>
         </table>  			
 ...
 <table width="100%" border="0" cellpadding="1" cellspacing="1" class="CTable">
              ...
              <tr>
                      ...
                      <td class="righttd">
                          <span style="display:none">会员每发表一条信息，可以得到 <asp:TextBox ID="PubAddMoney" Text="0" style="text-align:center" runat="server" Width="37px"></asp:TextBox>元人民币（为0时不奖励）<br /></span>
                          会员每发表一条信息，可以得到 <asp:TextBox CssClass="textbox" ID="PubAddPoint" Text="0" style="text-align:center" runat="server" Width="37px"></asp:TextBox><%=MyCache.GetCacheConfig(50) %><%=MyCache.GetCacheConfig(49) %>（为0时不奖励）
                         ...
                         <span style="color:red"><strong>温馨提示：</strong>
                             <br />1、如果这里设置为负数，表示投稿需要扣除点券或是积分；
                             <br />2、如果是奖励点券或积分，只有审核通过的稿件才奖励；如果是投稿要扣除，则不管稿件是否审核通过，一旦发表就扣除。
                         </span>
                      </td>
              </tr>
               <tr>
