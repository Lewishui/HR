...
<asp:Content ID="Content1" ContentPlaceHolderID="KSContent" Runat="Server">
    ...
    <script>
        ...
        $(function(){
            $(".iteminfo").each(function(){
                if ($(this).attr("defaultValue")!=''){
                    var cid=$('#'+$(this).attr('name')+'_classid').val()
                    LoadItemInfo($(this).attr('name'),cid,$(this).attr("bindModelID"),$(this).attr("defaultValue"))
                }
            });
        });
        ...
        $(function(){
           // $("#addToQQ").attr("checked",true);
          //  $("#addToQQLog").attr("checked",true);
           // $("#addToQQShuoShuo").attr("checked",true);
           // $("#addToSina").attr("checked",true);
            //$("input[name='defaultimg']").attr("checked",false);
           <%if (__modelinfo.BasicType==3 &&  ContentID==0){%>
            if ($("#serverid1 option:selected").val()!=0){
                $("#DownName1").val($("#serverid1 option:selected").text());
            }
            $("#serverid1").change(function(){
                if ($("#serverid1 option:selected").val()!=0){
                    $("#DownName1").val($("#serverid1 option:selected").text());
                }
            });
		...
		<div class="tab-page" id="ContentPanel<%=ChannelID %>">
         <script type="text/javascript">
           var tabPane1 = new WebFXTabPane( document.getElementById( "ContentPanel<%=ChannelID %>" ), 1 )
             ...
             <script>
                 var post_params={"AdminLoginCode":"<%=Kesion.Publics.KSCMS.GetCookie("Admin","AdminLoginCode") %>", "adminuser":"<%=Kesion.Publics.KSCMS.GetCookie("Admin","AdminUser") %>","adminpass":"<%=Kesion.Publics.KSCMS.GetCookie("Admin","AdminPass") %>","PowerList":"<%=Kesion.BLL.KSSecurity.ReturnPowerList() %>","usertype":"<%=Kesion.Publics.KSCMS.GetCookie("Admin","UserType") %>","CreateThumbs":1,"ChannelID":2,"AutoRename":4,"addwatermark":"1","fieldid":"-1","upbyswf":1};
             </script>
             <%}%>
			<%=JSStr %> 
        ...
        <tr>
            ...
            <div id='templatearea' style='display:none'>
             WEB模板：<asp:TextBox CssClass="textbox" Width="300px" ID="TemplateFile" runat="server"></asp:TextBox>
                <input class="button" onclick="SelectTemplate('<%=this.TemplateFile.ClientID %>',0);"
            ...
            <div<%=MyCache.GetCacheConfig(72)=="0"?" style='display:none'":"" %>>3G 模板：<asp:TextBox CssClass="textbox" Width="300px" ID="Template3GFile" runat="server"></asp:TextBox>
                <input class="button" onclick="SelectTemplate('<%=this.Template3GFile.ClientID %>',1);"
                    onmouseout="this.className='button';" onmouseover="this.className='button_on';"
                    type="button" value="选择模板..." />
                    </div>
              </div> 
       ...
       <td class="righttd">
		...
		<div class="tab-page" id="tab2">
		  <h2 class="tab" <%=__modelinfo.ChargesOption==1?"":" style='display:none'" %>>收费选项</h2>
            ...
            <script>
                function showGroup(){
                     if (jQuery("#<%=InfoPurview.ClientID%> :radio[checked]").val() == "2") {
                            jQuery("#<%=GroupPanel.ClientID%>").show();
                        } else {
                            jQuery("#<%=GroupPanel.ClientID%>").hide();
                        }
                }
    ...
    <table style="width:100%;margin:0 auto;" cellpadding="2" cellspacing="1" class="CTable">
     <tr>
      ...
      <td class="righttd">
          <asp:RadioButtonList ID="InfoPurview" runat="server">
              ...
              <asp:ListItem Value="1">所有会员（当所属栏目为非开放栏目，本文档对所有会员都开启阅读权限时，可以选择此项）</asp:ListItem>
              <asp:ListItem Value="2">指定会员组（当所属栏目为开放栏目，想单独对某些内容进行阅读权限设置，可以选择此项）</asp:ListItem>
          </asp:RadioButtonList>
           <div id="GroupPanel" runat="server" style="display:none;"></div>
         </td>
     </tr>
        <tr>
            <td align="right" class="lefttd">
                <strong>阅读<%=MyCache.GetChargeType(ChannelID)%>：</strong></td>
            <td class="righttd">
                      &nbsp;<asp:TextBox CssClass="textbox" ID="ReadPoint" Text="0" style="text-align:center" runat="server" Width="55px"></asp:TextBox>
                      <%=MyCache.GetChargeType(ChannelID,1)%> <span class="tips">免费阅读请设为 "<span style="color:red">0</span>"，否则获得上面设置权限的会员阅读本文档将消耗相应<%=MyCache.GetCacheConfig(49) %>，游客将无法阅读。</span>
            </td>
        </tr>
        <tr>
     ...
     <tr>
            ...
            <tr>
              ...
              <input name="editnum" type="hidden" id="editnum" value="0" /> <br /> <br />
