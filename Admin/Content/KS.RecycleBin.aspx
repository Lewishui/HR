...
function GetMyIds() {
    ...
    function ContentAddSpecial(){
        var sid = GetMyIds();
        if (sid!=null) parent.openWin("<%=itemName %>加入专题","<%=KSCMS.GetInstallDir()+MyCache.GetCacheConfig(4)%>/Content/KS.ContentAddSpecial.aspx?ContentID="+sid+"&ChannelID=<%=ChannelID%>",false,300,360);
    } 
...
function ContentMove() {
    var sid = GetMyIds();
    if (sid != null) parent.openWin("<%=itemName %>批量移动", "<%=KSCMS.GetInstallDir()+MyCache.GetCacheConfig(4)%>/Content/KS.ClassMove.aspx?ContentID=" + sid + "&ChannelID=<%=ChannelID%>", true, 600, 360);
...
function keyWordBlur() {
  ...
  //-->    
   ...
   <div id="manage_top" class="toptitle menu_top_fixed">
    <div class="quickSelect"><select name="classid" style="width:150px" OnChange="location.href='<%=KSCMS.GetInstallDir()+MyCache.GetCacheConfig(4)%>/Content/KS.RecycleBin.Aspx?classid='+this.value;">
