 ...
 <%if (string.IsNullOrEmpty(KSCMS.S("ContentID"))){ %>
    ...
    <table align="center" width="98%" border="0" cellpadding="1" cellspacing="1" class="CTable">
                ...
                <td colspan=3 class="righttd">
                 ...
                 <%if (string.IsNullOrEmpty(KSCMS.S("ContentID")))
                   { %>
                   <select multiple="multiple" size='18' style="WIDTH: 241px; " id="ClassID" name="ClassID">
                 ...
                 <td class="righttd"><select size='18'  style="WIDTH: 242px;" name="TargetClassID" id="TargetClassID">
                      <%=ClassManage.GetClassOptionByPurview(ChannelID,"0")%>
                    </select></td>
                 </tr>
    </table>
    ...
    <%}
       ...
       <asp:Button ID="BtnCancel" runat="server" Text="取消关闭" CssClass="button" OnClientClick="top.isreload=false;top.box.close()" />
       </div>
    <%} %>
    <br />  <%if (string.IsNullOrEmpty(KSCMS.S("ContentID"))){ %>
        <script type="text/javascript">
