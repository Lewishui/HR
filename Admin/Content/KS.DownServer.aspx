      ...
      <KS:KSGV ID="list" cssclass="CTable" runat="server" AutoGenerateColumns="False" PagerType="CustomNumeric"  Width="99%" EmptyDataText="没有添加任何服务器" GridLines="None" OnRowDataBound="list_RowDataBound" OnRowCommand="list_RowCommand" OnRowCreated="list_RowCreated" PageSize="20">
          <Columns>
             ...
             <asp:TemplateField HeaderText="是否默认">
                  <itemstyle horizontalalign="Center" width="40px" />
                 <ItemTemplate>
                     <%# ShowIsDefault(Convert.ToInt32(Eval("parentid")),Convert.ToInt16(Eval("islock"))) %>
                 </ItemTemplate>
             </asp:TemplateField>
             ...
             <itemtemplate>
             <div style="text-align:right" class="link-operate">
             <asp:LinkButton runat="server" id="addsub" commandargument='<%#Eval("downid") %>' commandname="addsub">添加下载服务器路径</asp:LinkButton>
             <asp:LinkButton runat="server" commandargument=<%#Eval("downid") %> commandname="modify">修改</asp:LinkButton>
          ...
          return true;
            ...
            function setdisabled(val)
            {
              ...
              if (val==0){
                  ...
                  jQuery('#<%=this.showDefault.ClientID%>').show();
              }
              else{
                  jQuery('#<%=this.ssss.ClientID%>').show();
                  jQuery('#<%=this.showDefault.ClientID%>').hide();
              }
            }
        $(function () {
           // setdisabled($('#servers option:selected').val());
            ...
            <table width="99%"  border="0" cellspacing="1" class="CTable" cellpadding="1" align=center>
             ...
             <tbody id="showDefault" runat="server">
               <tr>
                 <td class="lefttd" align="right"><b>是否默认选中：</b></td>
                 <td class="righttd"> 
