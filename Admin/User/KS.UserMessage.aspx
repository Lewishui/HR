<%@ Page Language="C#" ValidateRequest="false" MasterPageFile="~/Admin/MasterPage.master" AutoEventWireup="true" Inherits="Kesion.NET.WebSite.Admin_UserMessage" Title="站内消息管理" Codebehind="KS.UserMessage.aspx.cs" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="KSContent" Runat="Server">
 ...
 <asp:Panel ID="Panel1" runat="server">
     <div id="manage_top" class="menu_top_fixed">
          <ul>
            <li id='p3'><a href="javascript:;" onclick="if (GetIDS('del')){ $('#aspnetForm').attr('action','?action=del');$('#aspnetForm').submit()}"><b></b>删除选中</a></li>
            <li id='p1'><a href="KS.UserMessage.aspx?action=send"><b></b>发送站内消息</a></li>
          </ul>
        </div>     
...
<div class="content-area">
    ...
    站内短消息管理
      <div class="quickSearch">
        搜索记录：<asp:TextBox ID="TxtKeyWord" runat="server" CssClass="textbox" Width="200px" />
                 <asp:DropDownList ID="DrpSearchType" runat="server">
                     <asp:ListItem Value="0">接收者</asp:ListItem>
                     <asp:ListItem Value="1">发送者</asp:ListItem>
                     <asp:ListItem Value="2">标题</asp:ListItem>
                     <asp:ListItem Value="3">消息内容</asp:ListItem>
                 </asp:DropDownList>
                  <asp:RadioButtonList ID="RdbStatus" runat="server" RepeatDirection="Horizontal" RepeatLayout="Flow">
                      ...
                      <asp:ListItem Value="2">未读</asp:ListItem>
                  </asp:RadioButtonList>
  ...
  <div>
        <KS:KSGV ID="MessageView" cssclass="CTable" runat="server" AutoGenerateColumns="False" DataKeyNames="FieldID" GridLines="None"
            ...
            <Columns>
              ...
              <asp:TemplateField HeaderText="标题">
                <ItemTemplate>
                <span style="cursor:pointer" onmousedown='parent.openWin("查看短消息详情","<%=KSCMS.GetInstallDir()+MyCache.GetCacheConfig(4)%>/user/KS.UserMessageShow.aspx?ID="+<%# Eval("ID") %>,false);return false;'><%# Eval("MsgTitle") %></span>
                </ItemTemplate>
              </asp:TemplateField>
        ...
        <asp:Panel ID="Panel3" runat="server" Height="30px">
            ...
            <input type="button" value="删除选中的消息" onmouseover="this.className='button_on';" onmouseout="this.className='button';" class="button" onclick="if (GetIDS('del')){this.form.action='?action=del';this.form.submit()}" id="Button1" />             
            <input type="button" value="发送站内消息" onclick="location.href='KS.UserMessage.aspx?action=send';" class="button" />
</asp:Panel>  
        <br />
 ...
 <asp:Panel ID="Panel2" runat="server" Visible="false">
       ...
       <div id="manage_top" class="toptitle menu_top_fixed">
       	   发送站内短消息
       		<ul>
             <li id='p9'><b></b><a href='KS.UserMessage.aspx'>返回</a></li> 
           </ul>   
	  </div>
      <div class="menu_top_fixed_height"></div> 
<div class="content-area">
     ...
     <table width="100%" border="0" cellpadding="1" cellspacing="1" class="CTable"> 
          <tr>
            ...
            <td class="righttd">
                <input type="radio" name="optype" onclick="jQuery('#grouparea').hide();" value="1"<%=string.IsNullOrEmpty(KSCMS.S("userid"))?" checked":"" %> />所有会员(<span style="color:red"><%=DataFactoryUser.GetOnlyValue("select count(1) From KS_User") %></span>位)<br />
                ...
                <input type="radio" name="optype" onclick="jQuery('#grouparea').hide();" value="3"<%=!string.IsNullOrEmpty(KSCMS.S("userid"))?" checked":"" %> />指定用户名<asp:TextBox ID="ToUserName" runat="server"
                    Width="246px" CssClass="textbox"></asp:TextBox><span class="tips">多个用户名间请用<font color="#0000ff">英文的逗号</font>分隔</span></td>
          </tr>
         <tr>
