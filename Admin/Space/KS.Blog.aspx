...
<asp:Content ID="Content1" ContentPlaceHolderID="KSContent" Runat="Server">
    <div  id="manage_top" class="menu_top_fixed">
         <ul>
              <li id='p2'>
              <asp:LinkButton ID="addclassbutton" runat="server" OnClick="addclassbutton_Click1" ><b></b>空间管理</asp:LinkButton></li>
              <li id='p1'>
              <asp:LinkButton ID="delclassbutton" runat="server" onclick="delclassbutton_Click" ><b></b>空间分类</asp:LinkButton></li>
         </ul>
    </div> 
...
<asp:Panel ID="listPanel" runat="server">
     <div class="myTitle">
     	个人空间管理
         <div class="quickSearch"><table border="0" cellpadding="0" cellspacing="0" width="100%">
              <tr>
    ...
    <KS:ksgv id="List" cssclass="CTable" runat="server" AutoGenerateColumns="False" EmptyDataText="没有空间记录!" PagerType="CustomNumeric" Width="99%" GridLines="None" OnRowDataBound="List_RowDataBound" OnRowCommand="List_RowCommand" OnRowCreated="List_RowCreated" >
            <Columns>
                ...
                <asp:TemplateField HeaderText="站点名称" HeaderStyle-HorizontalAlign="center" >
                    <ItemTemplate>
                        <a href="<%=KSCMS.GetInstallDir() %>space/?<%# Eval("UserID") %>" target="_blank" style="display:block; height:30px; line-height:30px;" ><img src="<%# (Kesion.HtmlTags.Function.FileOrDirectoryIFExist(Eval("Logo").ToString()))? Eval("Logo") : Kesion.Cache.MyCache.GetCacheConfig(5) + Kesion.Cache.MyCache.GetCacheConfig(0) + "/SysImg/nopic.gif" %>" width="30" height="30" style=" vertical-align:middle" /> <%# Eval("BlogName") %></a>
                    </ItemTemplate>
                </asp:TemplateField>
            ...
            <asp:TemplateField HeaderText="状态">
              ...
              <itemtemplate>
                   <a href="?action=verify&blogid=<%#Eval("blogID")%>" <%# Utils.StrToInt(Eval("status"))==0?"class='no' title='未审核'":"class='ok' title='已审核'" %>></a>
              </itemtemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="推荐">
              <itemstyle width="30px" horizontalalign="Center"/>
              <itemtemplate>
                   <a href="?action=recommend&blogid=<%#Eval("blogID")%>" <%# Utils.StrToInt(Eval("Best"))==0?"class='no' title='否'":"class='ok' title='是'" %>></a>
              </itemtemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="热门">
              <itemstyle width="30px" horizontalalign="Center"/>
              <itemtemplate>
                   <a href="?action=hot&blogid=<%#Eval("blogID")%>" <%# Utils.StrToInt(Eval("hot"))==0?"class='no' title='否'":"class='ok' title='是'" %>></a>
              </itemtemplate>
            </asp:TemplateField>
            <asp:TemplateField HeaderText="置顶">
              <itemstyle width="30px" horizontalalign="Center"/>
              <itemtemplate>
                   <a href="?action=Toplist&blogid=<%#Eval("blogID")%>" <%# Utils.StrToInt(Eval("Toplist"))==0?"class='no' title='否'":"class='ok' title='是'" %>></a>
              </itemtemplate>
            </asp:TemplateField>
