...
<script>
    function addWorkTable() {
        parent.openWin("添加快捷键到我的工作台", "<%=KSCMS.GetInstallDir()+MyCache.GetCacheConfig(4)%>/System/KS.WorkTableManage.aspx?action=add", true, 820, 500);
    }
    function delWorkTable(id) {
        KesionJS.Confirm("确定删除该快捷键?", "location.href='<%=KSCMS.GetInstallDir()+MyCache.GetCacheConfig(4)%>/System/KS.WorkTableManage.aspx?action=delQuickMenu&from=FrameWork/Main.aspx&ids=" + id + "';", null);
    }
    function addPlan() {
        parent.openWin("添加工作计划", "<%=KSCMS.GetInstallDir()+MyCache.GetCacheConfig(4)%>/User/KS.AdminPlan.aspx?action=add", true, 820, 450);
    }
    function rz() {
        $(".concent .left").width($(window).width() - $(".right").width() - 60);
    }
    $(function () {
        rz();
...
<body class="prebody">
  ...
  <div class="concent">
     ...
     <div id="manage_top" class="menu_top_fixed">
        <div class="mainarea" style="padding-left:15px;">
           <span><label id="AdminLoginInfo" runat="server"></label></span>
        </div>
     </div>
     ...
     <div class="index-main"> 
      <div class="left">
         <div class="quickly"<%=KSSecurity.CheckMenuPower(KSSecurity.RemovePowerBadChar("System/KS.WorkTableManage.aspx")).ToString().ToLower().Equals("true")?"":" style='display:none'" %>>   
              ...
              using (DataTable dt = MyCache.GetCacheModelTable())
               {
                ...
                num = Utils.StrToInt(DataFactoryUser.GetOnlyValue("select count(1) From KS_User where datediff(day,[regdate],getdate())=0"));
                   ...
                   if (dt.Rows.Count > 0)
                   {
                       foreach (DataRow dr in dt.Rows)
                       {
                           if (dr["modelstatus"].ToString() == "1")
                           {
                               if (dr["basictype"].ToString() != "4" || (dr["basictype"].ToString() == "4" &&  (Utils.GetSysInfo("//sysinfo/model/mall").ToString().ToLower() == "true")))
                               {
                                   num = Utils.StrToInt(DataFactory.GetOnlyValue("select count(1) From " + dr["ModelTable"].ToString() + " where deltf=0 and verify=0"));
...
<% 
              ...
              Response.Write(string.Format(@"<li title=""评论系统"">评论总数<span>{0}</span></li>", num > 0 ? @"<a href='../Content/KS.Comment.Aspx' class='has'>" + num + "</a>" : num.ToString()));
                ...
                num = Utils.StrToInt(DataFactoryUser.GetOnlyValue("select count(1) From KS_User"));
      ...
      <div class="right">
         ...
         <div class="rili">
         	<div class="gzjh_box">
          <input type="button" style="padding:10px;text-align:center;width:100%" onClick="addPlan()" value="写工作计划" class="gzjh_btn" />
          </div>

             <style>
			    .gzjh_box{ position:relative; margin-bottom:10px;}
				.gzjh_btn{ height:44px; color:#000;background:#fff;
				outline:0;cursor:pointer; border:#eee solid 1px;font-size:16px;border-radius: 5px;}
				.gzjh_box label{background: url(/admin/images/all_icon.png) no-repeat -403px -505px; display:inline-block; height:27px; width:27px; position:absolute; top:9px; left:29px;}
                 .today {
                     background:#FFBB00; font-size:16px; border:0;
                 }
				 ...
				 .week{ height:30px; font-weight:normal; padding:0 2px;}
				 .titlestyle{ border:0}
                 .titlestyle td{
...
TodayDayStyle-CssClass="today" DayStyle-CssClass="otherday" DayHeaderStyle-CssClass="otherday" BorderWidth="0px"
...
<DayHeaderStyle HorizontalAlign="Center" CssClass="otherday week" Font-Size="12px"></DayHeaderStyle>

<DayStyle CssClass="otherday" Font-Size="13px"></DayStyle>

<OtherMonthDayStyle ForeColor="#CCCCCC"></OtherMonthDayStyle>

<TitleStyle CssClass="titlestyle" Font-Size="13px" BackColor="#F4F6F7" BorderWidth="0"></TitleStyle>
...
<TodayDayStyle CssClass="today"></TodayDayStyle>
         ...
         <div class="rizhi">
           <div class="tit"><i></i>查看工作计划</div>
            ...
            <div class="copyright" style="margin:0 auto; padding:20px 0;width:80%;"/>
              <%=KSCMS.CopyRight() %>
        </div>
