...
<head runat="server">
    <title>上传</title>
     ...
     <script type="text/javascript" src="/admin/images/wCheck.min.js"></script>  
    <script type="text/javascript">  
        $(document).ready(function() {
            $("input[type='checkbox']").removeAttr("id").wCheck({ theme: 'square-classic-red', selector: 'checkmark' });
        })
    </script>  
</head>
    ...
    { %>
     ...
     <%if (UpType == "UpByBar")  //上传（带进度条）
       ...
       <script>
			...
			var post_params={"AdminLoginCode":"<%=KSCMS.GetCookie("Admin","AdminLoginCode") %>", "adminuser":"<%=KSCMS.GetCookie("Admin","AdminUser") %>","adminpass":"<%=KSCMS.GetCookie("Admin","AdminPass") %>","PowerList":"<%=KSSecurity.ReturnPowerList() %>","usertype":"<%=KSCMS.GetCookie("Admin","UserType") %>","upBySwf":1,ChannelID:<%=channelId%>,"UpType":"<%=UpType%>","FieldID":"<%=fieldId%>","id":"<%=Utils.StrToInt(KSCMS.S("id"))%>","CurrPath":"<%=CurrPath %>","AutoRename":4};
		...
		<table border='0' cellpadding="0" cellspacing="0">
		 <tr><td><div class="uptips" id="showspeed"><div class="upbutton" id="shows"><span id="spanButtonPlaceholder"></span></div></div></td>
      ...
      %>
		...
		window.onload = function () {
			...
			swfu = new SWFUpload({
				...
				post_params: {"adminUserFaceID":"<%=adminUserFaceID%>", "AdminLoginCode":"<%=KSCMS.GetCookie("Admin","AdminLoginCode") %>","adminuser":"<%=KSCMS.GetCookie("Admin","AdminUser") %>","adminpass":"<%=KSCMS.GetCookie("Admin","AdminPass") %>","PowerList":"<%=KSSecurity.ReturnPowerList() %>","usertype":"<%=KSCMS.GetCookie("Admin","UserType") %>","upBySwf":1,UpType:"<%=UpType%>",module:"<%=module%>",ChannelID:<%=channelId%>,"id":"<%=Utils.StrToInt(KSCMS.S("id"))%>","FieldID":"<%=fieldId%>","ispic":<%=isPic%>,"AutoRename":4,"AddWaterMark":<%=AddWaterMark%>,"CreateThumbs":<%=CreateThumbs%><%if (isPic==1){ %>,"ThumbsWidth":jQuery('#PicWidth').val(),"ThumbsHeight":jQuery('#PicHeight').val()  <%} %>},
		...
		<table cellspacing="0" cellspadding="0" border="0">
		 <tr>
		  <td width="<%=btnWidth%>">
		  <div class="upbutton" style="width:<%=btnWidth%>px;height:<%=btnHeight%>px;line-height:<%=btnHeight%>px;margin-top:2px;"><span id="spanButtonPlaceholder">选择文件</span></div>
		 </td>
	<%
           if (!string.IsNullOrEmpty(module))
           {
     ...
     if (isPic==1)
      {%>
		  <td>&nbsp;<label><input type="checkbox" name="DefaultUrl" id="DefaultUrl" onClick="SetDefaultUrl(this)" value="1"<%=CreateThumbs==1?" checked":string.Empty %>>缩略图</label> 
            <%if (ConfigManage.GetConfig().WaterMarkStatus!=0){ %>     
          <label><input name="AddWaterFlag" type="checkbox" id="AddWaterFlag" onClick="SetAddWaterFlag(this)" value="1"<%=AddWaterMark==1?" checked":string.Empty %>>水印</label> 
              <%} %>
             <%
          ...
          %>
		  <strong>缩略图大小：</strong>宽<input name="PicWidth" onKeyUp="SetThumbWidth()" id="PicWidth" class="textbox" style="width:40px;text-align:center" value="<%=thumbWidth %>">PX 高<input name="PicHeight" id="PicHeight" class="textbox" onKeyUp="SetThumbHeight()" style="width:40px;text-align:center" value="<%=thumbHeight%>">PX
