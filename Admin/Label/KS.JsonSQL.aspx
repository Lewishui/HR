<%@ Page Language="C#" ValidateRequest="false"  MasterPageFile="~/Admin/MasterPage.master" AutoEventWireup="true" Inherits="Kesion.NET.WebSite.Admin_Label_JsonSQL" Title="自建Json数据" Codebehind="KS.JsonSQL.aspx.cs" %>
...
<script type="text/javascript">
  ...
  {
      jQuery.get('KS.JsonSQL.aspx',{action:'CheckLabelName',rnd:Math.random(),labeltype:'<%=labeltype %>',labelid:'<%=labelid %>',labelname:labelname},function(s){
    if (jQuery("#<%=this.LabelName.ClientID %>").val()==''){
	 jQuery('#labelmessage').html('<font color=red>请输入json名称!</font>');
	 lbtf=false;
	}
	else if (s=='true'){
  ...
  {
   jQuery.get('KS.JsonSql.aspx',{action:'CheckDataSource',lst:jQuery('#<%=this.LabelDataSource.ClientID %>>option:selected').val(),cstr:jQuery('#<%=this.ConnStr.ClientID %>').val()},function(s){
     if(s=='true')
     {
  ...
  function checkform()
  {
   ...
   var str='{KS:Label Type="ISQL"'
          ...
          + ' PageStyle="1"'
          + ' CacheTime="<%=this.TxtCacheTime.Text%>"'
          + ' ItemName="1"'
   ...
   {  
	...
	var link="label/KS.GetFieldParam.aspx?from=json&m_fieldname=" + fieldname+"&fieldtype="+fieldtype;
...
<KS:TopNav runat="server" Text="创建json数据第一步" ID="TopNav1" />
    ...
    <asp:Panel ID="Step1Panel" runat="server" Width="100%">
        ...
        <tr>
            <td class="lefttd" style="height: 30px; text-align: right">
                <strong>json名称：</strong></td>
            <td id="Td2" runat="server" class="righttd" style="height: 30px; text-align: left">
              <asp:TextBox CssClass="textbox" ID="LabelName" runat="server" Width="250px"></asp:TextBox><span style="color: #000066">  *<asp:RequiredFieldValidator ID="RequiredFieldValidator1" runat="server" ControlToValidate="LabelName"
                    Display="Dynamic" ErrorMessage="Json名称必须输入"></asp:RequiredFieldValidator><span id="labelmessage"></span>
                例如："推荐文章列表",“最新热门列表”等等</span></td>
        </tr>
    <tr>
     <td class="lefttd" style="text-align: right; height: 30px;">
         <strong>json分类：</strong></td>
     <td class="righttd" style="text-align: left; height: 30px;"><span  id="classidarea" runat="server"></span><font color=red> 请选择标签所属目录，以便日后管理标签</font></td>
        ...
        <tr id="printoutarea" runat="server">
            ...
            <tr id="labeltypearea" runat="server">
                ...
                <td class="righttd" style="height: 30px; text-align: left">
                    <asp:RadioButtonList ID="SQLType" runat="server" RepeatDirection="Horizontal">
                        <asp:ListItem Selected="True" Value="0">普通json</asp:ListItem>
                        <asp:ListItem Value="1">分页json</asp:ListItem>
                    </asp:RadioButtonList>
                    </td>
            </tr>
            ...
            <tr>
                ...
                <td class="righttd" style="height: 30px; text-align: left">
                    <asp:TextBox ID="EmptyText" CssClass="textbox" TextMode="MultiLine" Text="对不起，本栏目下没有任何信息！" runat="server" Width="550px" Height="100"></asp:TextBox></td>
            </tr>
            
             <tr>
                <td class="lefttd" style="height: 30px; text-align: right">
                    <strong>数据缓存时间：</strong></td>
                <td class="righttd" style="height: 30px; text-align: left">
                    <asp:TextBox ID="TxtCacheTime" CssClass="textbox" Text="0" runat="server" Width="60px" style="text-align:center"></asp:TextBox>分钟
                    <span class="tips">Tips:设置为“0”不启用缓存，设置为“-1”永久缓存。对查询出来的数据没有经常更新的标签，建议设置一个缓存时间，减少数据库查询，整个系统的性能将得到很好的提升。</span>
                </td>
            </tr>
            ...
            <tr id="showsql" runat="server" visible="false">
                ...
                <td class="righttd" style="height: 30px; text-align: left">
                    <asp:TextBox ID="TxtSQL1" TextMode="MultiLine" CssClass="textbox textarea" Text="" runat="server" Width="600px" Height="70px"></asp:TextBox>
                          <div class="message">
...
如：http://www.kesion.com/index.aspx?ClassID=100,那么sql语句：select top 10 classname from ks_class where classid=<font color=blue>[ReqNum(ClassID)]</font> 将自动转换为select top 10 classname from ks_class where classid=<font color=blue>100</font><br />
       2、可以用标签[KS:Current:UserName]和[KS:Current:UserID]分别得到当前用户名和用户UserID
      </div>
        ...
        <asp:Panel ID="Step2Panel" runat="server" Visible="false" Width="100%">
            ...
            <tr>
                ...
                <td id="Td3" runat="server" class="righttd" colspan="2" style="height: 30px; text-align: left; width: 27px;">
                    <asp:ListBox ID="field" runat="server" Rows="15" Width="320px" SelectionMode="Multiple"></asp:ListBox></td>
                <td runat="server" class="righttd" style="height: 30px; text-align: center; width: 78px;" id="Td4">
                    &gt;&gt;&gt;</td>
                <td class="righttd">
                    <asp:ListBox ID="field2" runat="server" Rows="15" Width="320px" SelectionMode="Multiple"></asp:ListBox></td>
            </tr>
                    <tr id="labelparamarea" runat="server" visible="false">
                        <td class="lefttd" style="height: 30px; text-align: right">
            ...
            <tr>
                ...
                <td runat="server" class="righttd" colspan="4" style="height: 30px; text-align: left" id="Td9">
			        ...
			        <table border='1' cellpadding='2' cellspacing='1'  width='100%' class='CTable' style="display:none">
			        <tr align='center'>
     ...
     <div style="text-align:center">
        ...
        <tr>
            ...
            <td id="Td8" runat="server" class="righttd" style="height: 30px; text-align: left">
             ...
             <tr id="labelparamarea1" runat="server" visible="false">
                 <td class="lefttd" style="height: 30px; text-align: right">
        ...
        <tr>
                <td class="lefttd" style="height: 30px; text-align: right">
                    ...
                    <span style="background-color: #f0f9fe">n取值范围0-6之间,</span><a href="#" onclick="purview();return false"><font
                        color="blue">预览</font></a><br /><br />
                    分页标签可以用标签[KS:TotalPageNum]和[KS:TotalRecordNum]分别得到总页数和总记录数
                    &nbsp;&nbsp;
                    </td>
         ...
         <asp:HiddenField ID="HidTemplateID" runat="server" Value="0" />
