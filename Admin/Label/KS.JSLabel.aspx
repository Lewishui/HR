...
<script language="javascript">
   ...
   {  
        parent.openWin("标签目录管理","<%=Kesion.Publics.KSCMS.GetInstallDir()+Kesion.Cache.MyCache.GetCacheConfig(4)%>/label/KS.LabelClass.aspx?parentid="+parentid+"&classid="+classid+"&classtype=<%=LabelType %>&templateID=<%=templateId%>",true,620,350);
    } 
...
<ul> <span style="float:right;"><%=templateName %></span>
     <span style="float:left;padding-right:10px;">
     <select onchange="location.href='KS.JSLabel.aspx?templateId=<%=templateId %>&labelType='+this.value">
         <option value="">JS标签类型</option>
         <option value="5"<%if (KSCMS.S("labelType").Equals("5")) { Response.Write(" selected"); } %>>自定义静态JS</option>
         <option value="6"<%if (KSCMS.S("labelType").Equals("6")) { Response.Write(" selected"); } %>>智能样式JS（SQL）</option>
     </select>
    </span>
      ...
      <a href="#" onclick="addclass(<%=ClassID %>,0);return false;" title="创建标签分类"><b></b>新建分类</a></li><%if (LabelType==5) 
        {%><li id='p8'><a href="KS.JS.Var.aspx?LabelType=<%=LabelType %>&ClassID=<%=ClassID %>&templateID=<%=templateId %>"><b></b>新建静态JS</a></li><%
        }
        else
        { %><li id='p8'><a href="KS.Label.SQL.aspx?LabelType=<%=LabelType %>&ClassID=<%=ClassID %>&templateID=<%=templateId %>"><b></b>新建智能JS</a></li><%} %><li id='p9'><a href="?LabelType=<%=LabelType %>&ClassID=<%=ParentID %>&templateID=<%=templateId %>"><b></b>回上一级</a></li></ul>
  </div>
                ...
                <Columns>
                 ...
                 <itemstyle horizontalalign="Center" />
                    ...
                    <itemtemplate>
                      ...
                      <a href='?Action=del&id=<%#Eval("classid") %>&orderid=<%#Eval("orderid") %>&labeltype=<%=LabelType %>&ClassID=<%=ClassID %>&templateId=<%=templateId %>'  onclick="return(confirm('此操作不可逆，确定删除操作吗?'))">删除</a>
                     <br />
